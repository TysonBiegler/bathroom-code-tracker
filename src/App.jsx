...
export default function BathroomCodeTracker() {
  ...
  const getCurrentLocation = (desiredAccuracy = 50, maxAttempts = 5) => {
      ...
      const tryGetPosition = () => {
        ...
        navigator.geolocation.getCurrentPosition(
          async (position) => {
            ...
            console.log(
              `Attempt ${attempts}: lat=${latitude}, lon=${longitude}, accuracy=${accuracy}m`,
            );

            // If the accuracy is good enough or we've tried maxAttempts
            if (accuracy <= desiredAccuracy || attempts >= maxAttempts) {
              ...
              try {
                ...
                const response = await fetch(
                  ...
                  { headers: { "User-Agent": "BathroomCodeTracker/1.0" } },
                );
                if (response.ok) {
              ...
              // Accuracy too low, retry
              console.log("Accuracy too low, retrying...");
    ...
    if (button) {
      ...
      try {
        ...
        if (error.message.includes("unavailable")) {
          userMessage =
            "Unable to determine your location. This could be because:\n\n" +
  ...
  const handleSubmit = (e) => {
    ...
    const entry = {
      ...
      firstAdded: editingId
        ? entries.find((e) => e.id === editingId)?.firstAdded || now
        : now,
      lastUpdated: now,
    };
