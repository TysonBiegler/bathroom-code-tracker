...
export default function BathroomCodeTracker() {
  ...
  const EntryCard = ({ entry, showDistance = false }) => {
    ...
    return (
      <>
        <div
          className={`entry-card-compact ${isExpanded ? "expanded" : ""}`}
        ...
        >
          ...
          <div className="entry-meta">
            <span
              title={`Added: ${formatDate(entry.firstAdded)}\nUpdated: ${formatDate(entry.lastUpdated)}`}
            >
            ...
            <div className="modal-card" onClick={(e) => e.stopPropagation()}>
              ...
              {showDistance && entry.distance !== undefined && (
                <p className="modal-distance">
                  {entry.distance < 0.1 ? "<0.1" : entry.distance.toFixed(1)}{" "}
                  miles away
                </p>
              )}
              ...
              <div className="modal-meta">
                <div>
                  <strong>First Added:</strong> {formatDate(entry.firstAdded)}
                </div>
                <div>
                  <strong>Last Updated:</strong> {formatDate(entry.lastUpdated)}
                </div>
              </div>
